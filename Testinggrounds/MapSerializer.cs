...
namespace IsometricWPF
{
    ...
    /// <summary>
    /// Bug #2 fix: replaces SerializedCell[,] (unsupported by System.Text.Json) with a flat list.
    /// Each entry carries its own X/Y coordinates.
    /// </summary>
    ...
    public static class MapSerializer
    {
        ...
        public static void Export(WorldMap map, IEnumerable<DwellerInstance> dwellers, string path)
        {
            ...
            // Cells as flat list (skips empty cells to keep files small)
        ...
        public static (WorldMap map, List<DwellerInstance> dwellers, bool ok, string? error) Import(string path)
        {
            try
            {
                ...
                foreach (var entry in data.Cells ?? new())
                {
