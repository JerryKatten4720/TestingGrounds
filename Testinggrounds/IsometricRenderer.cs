...
public sealed class IsometricRenderer {
    ...
    private readonly Brush _fogDarkBrush;
    private readonly Brush _fogSeenBrush;
    ...
    private readonly Brush _radBrush;
    ...
    public IsometricRenderer() {
        ...
        _tileVisual = Host.AddVisual();

        _gridPen = new Pen(new SolidColorBrush(Color.FromArgb(55, 0, 0, 0)), 0.5);
        _gridPen.Freeze();
        _highlightFill = new SolidColorBrush(Color.FromArgb(50, 80, 220, 80));
        _highlightFill.Freeze();
        _highlightPen = new Pen(new SolidColorBrush(Color.FromArgb(140, 80, 255, 80)), 1.0);
        _highlightPen.Freeze();
        _pathPen = new Pen(new SolidColorBrush(Color.FromArgb(200, 255, 220, 0)), 2.0);
        _pathPen.Freeze();

        _fogDarkBrush = new SolidColorBrush(Color.FromRgb(18, 18, 28));
        _fogDarkBrush.Freeze();
        _fogSeenBrush = new SolidColorBrush(Color.FromArgb(205, 0, 0, 0));
        _fogSeenBrush.Freeze();
        _radBrush = new SolidColorBrush(Color.FromArgb(60, 80, 220, 0));
        _radBrush.Freeze();
    }
    ...
    public DrawingVisualHost Host { get; }
    ...
    public void SetShowHpBars(bool on) {
        _showHpBars = on;
        Redraw();
    }
    ...
    public void SetMovementHighlight(IReadOnlySet<(int, int)>? tiles) {
        _moveHighlight = tiles;
    }

    public void SetPathPreview(IReadOnlyList<(int, int)>? path) {
        _pathPreview = path;
    }
    ...
    private void DrawDarkTile(DrawingContext dc, int gx, int gy) {
        TileToScreen(gx, gy, out var sx, out var sy);
        var geo = GetTopGeo(0);
    ...
    private void DrawFogOverlay(DrawingContext dc, int gx, int gy, Brush brush) {
        TileToScreen(gx, gy, out var sx, out var sy);
        var top = _map![gx, gy].MaxBlockHeight;
        var heightPx = _showHeights && top >= 0 ? (top + 1) * StackStep : 0;
        var t = sy - heightPx;
    ...
    private void DrawCell(DrawingContext dc, int gx, int gy) {
        var cell = _map![gx, gy];
        TileToScreen(gx, gy, out var sx, out var sy);
        var pen = _showGrid ? _gridPen : null;
        ...
        if (_showHeights && cell.Blocks.Count > 0) {
            foreach (var kvp in cell.Blocks.OrderBy(b => b.Key)) {
                ...
                var topY = -(hi + 1) * StackStep;

                dc.DrawGeometry(brs[1], pen, GetSideGeo(topY, StackStep + 0.5, true));
                dc.DrawGeometry(brs[2], pen, GetSideGeo(topY, StackStep + 0.5, false));
        ...
        if (cell.Decors.Count > 0) {
            var sh = _showHeights ? cell.Blocks.Count * StackStep : 0;
    ...
    private void DrawCombatOverlays(DrawingContext dc, int gx, int gy) {
        var hl = _moveHighlight?.Contains((gx, gy)) == true;
        var path = _pathPreview?.Contains((gx, gy)) == true;
        if (!hl && !path) return;

        TileToScreen(gx, gy, out var sx, out var sy);
        var top = _map?[gx, gy].MaxBlockHeight ?? -1;
        var heightPx = _showHeights && top >= 0 ? (top + 1) * StackStep : 0;
        var t = sy - heightPx;
        ...
        if (path) {
            var pf = new SolidColorBrush(Color.FromArgb(80, 255, 220, 0));
            pf.Freeze();
    ...
    private void DrawPathPreview(DrawingContext dc) {
        ...
        for (var i = 0; i < _pathPreview.Count - 1; i++) {
    ...
    private Brush ProjectBrush(Brush b, FaceType face) {
        var factor = face switch { FaceType.Top => 1.0f, FaceType.Left => 0.8f, _ => 0.6f };
        ...
        if (factor < 1.0f) {
            ...
            fin = new DrawingBrush(dg) {
                ViewportUnits = BrushMappingMode.Absolute, Viewport = vp, TileMode = TileMode.Tile,
                Stretch = Stretch.Fill, Transform = group
            };
        }
        else {
            fin = new ImageBrush(img.ImageSource) {
                ViewportUnits = BrushMappingMode.Absolute, Viewport = vp, TileMode = TileMode.Tile,
                Stretch = Stretch.Fill, Transform = group
            };
        }
    ...
    public void OnMouseMove(Point worldPos) {
        ScreenToTile(_map, worldPos.X, worldPos.Y, out var gx, out var gy);
        if (_map == null || !_map.IsInBounds(gx, gy)) {
            if (_lastHoverX >= 0) {
                TileHoverLeft?.Invoke(_lastHoverX, _lastHoverY);
                _lastHoverX = -1;
            }
    ...
    public void ScreenToTile(WorldMap? map, double wx, double wy, out int gx, out int gy) {
        ...
        for (var h = AppConfig.Instance.MaxStackHeight; h >= 1; h--) {
            var curWy = wy + h * StackStep;
            var ax = wx - TileW / 2.0;
            var tx = (int)Math.Floor((ax / (TileW / 2.0) + curWy / (TileH / 2.0)) / 2.0);
            var ty = (int)Math.Floor((curWy / (TileH / 2.0) - ax / (TileW / 2.0)) / 2.0);
    ...
    public PointCollection GetDiamondPoints(int gx, int gy) {
        ...
        return new PointCollection {
            new Point(sx + TileW / 2, t),
            new Point(sx + TileW, t + TileH / 2),
            new Point(sx + TileW / 2, t + TileH),
            new Point(sx, t + TileH / 2)
        };
    }
    ...
    private Dictionary<long, List<DwellerInstance>> BuildDwellerLookup() {
        ...
        foreach (var d in _dwellers) {
            var k = ((long)d.TileX << 32) | (uint)d.TileY;
            if (!lookup.TryGetValue(k, out var list)) lookup[k] = list = new List<DwellerInstance>();
            list.Add(d);
        }
